...
interface HistoryListFormValues {
  name?: string;
  startDate?: string;
  finalDate?: string;
}
...
const HistoryList = () => {
  ...
  const formMethods = useForm<HistoryListFormValues>({ mode: "all" });
  const {
    handleSubmit,
    register,
    formState: { errors },
  ...
  const {
    data: historicoUsuario,
    isFetching: isFetchingHistoricoUsuarios,
    isError: isErrorHistoricoUsuario,
  } = useGetHistoricosUsuarioQuery(user?.id as number, {
    skip: isAdmin,
  });
  ...
  const isLoading = isFetchingHistoricoUsuarios || isFetchingHistoricos;
  const isError = isErrorHistoricoUsuario || isErrorHistoricos;
  const listToShow = isAdmin ? historicos : historicoUsuario;
    ...
    <Box component={Paper} display="flex" flexDirection="column" p={2}>
      ...
      {isLoading ? (
        ...
        <Box
          ...
          justifyContent="center"
        >
          <Typography fontWeight="bold">Não há histórico.</Typography>
        </Box>
      ) : (
        <TableSchedulingLists
          data={filteredList as Array<GetHistoricosResult>}
          listType="history"
        />
      )}
    </Box>
