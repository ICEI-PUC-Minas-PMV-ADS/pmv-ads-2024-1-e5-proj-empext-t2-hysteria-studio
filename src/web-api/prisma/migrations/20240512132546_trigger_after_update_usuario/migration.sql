...
-- This is an empty migration.
-- This is an empty migration.
DROP TRIGGER IF EXISTS `railway`.`usuario_AFTER_UPDATE`;

CREATE DEFINER = CURRENT_USER TRIGGER `railway`.`usuario_AFTER_UPDATE` AFTER UPDATE ON `Usuarios` FOR EACH ROW
BEGIN
